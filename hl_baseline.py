...
x_train = features[:78, :]  # 80, 78
hl_train = hl_label[:78]
x_test = features[78:, :]
hl_test = hl_label[78:]
x_train_norm = minmax_scaler.fit_transform(x_train)
x_test_norm = minmax_scaler.transform(x_test)
hl_train_norm = std_scaler.fit_transform(hl_train.reshape(-1, 1))
hl_test_norm = std_scaler.transform(hl_test.reshape(-1, 1))

regressor_rf_hl = RandomForestRegressor(n_estimators=300, max_depth=10, random_state=seed, min_samples_leaf=1)
...
# print("MAE score:\n", np.mean(cl_mae))
# print("MSE score:\n", np.mean(cl_mse))
regressor_rf_hl.fit(x_train, hl_train)
rf_cl_pred = regressor_rf_hl.predict(x_test)


regressor_mlp_hl = MLPRegressor(hidden_layer_sizes=(512, ), activation='relu', solver='lbfgs', learning_rate='adaptive',
                             learning_rate_init=1e-3, shuffle=True, random_state=seed, max_iter=500, batch_size=32)
regressor_mlp_hl.fit(x_train_norm, hl_train_norm)
mlp_hl_pred = regressor_mlp_hl.predict(x_test_norm)
mlp_hl_pred = std_scaler.inverse_transform(mlp_hl_pred.reshape(-1, 1))

rf_mse = mean_squared_error(hl_test, rf_cl_pred)
rf_mae = mean_absolute_error(hl_test, rf_cl_pred)

mlp_mse = mean_squared_error(hl_test, mlp_hl_pred)
mlp_mae = mean_absolute_error(hl_test, mlp_hl_pred)

rf_result = [hl_test, rf_cl_pred]
...
mlp_pk_pred = mlp_hl_pred.squeeze(1)
mlp_result = [hl_test, mlp_pk_pred]
...
feature_names = ['Blood Group', 'Height', 'Weight', 'BMI', 'FFM', 'Age','Dose/IU', 'IU/KG','VWF-Ag',
                 'pre', 'timst', '24fviii']
importance = regressor_rf_hl.feature_importances_
...
ax.set_title('feature importance by RF regressor for half-life ', fontsize=16)
