...
def kfold_val(model,fold):
    ...
    for train_index, test_index in kf.split(norm_features):
        ...
        X_train, y_train = norm_features[train_index], norm_ivr[train_index]
        X_test, y_test = norm_features[test_index], norm_ivr[test_index]
...
if __name__ == '__main__':
    ...
    d_token = 192  # embedding size [96, 128, 192, 256, 320, 384] 192
    n_blocks = 3  # 3,4,6
    attention_dropout = 0.3  # 0.4, 0.3
    ffn_factor = 6 / 3  # 2/3, 4/3, 6/3,
    ffn_d_hidden = int(d_token * ffn_factor)
    ffn_dropout = 0.0  # 0.0, 0.05
    ...
    model = rtdl.FTTransformer.make_baseline(
        n_num_features=12,  # blood group and timst to predict are categorical features 16, 14, 12
