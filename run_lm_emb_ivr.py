...
def get_training_dataset(path, indices, use_reg=False):
    ...
    for i in indices:
        ...
        x_inputs.append(
            " and ".join([f"{x_columns[idx]} is {_}" for idx, _ in enumerate(x_features)])
        )
        if not use_reg:
...
if __name__ == "__main__":
    ...
    for dataset_i, current_dataset in enumerate(all_datasets_path):
        ...
        if "FVIII_IVR_processed" not in current_dataset:
        ...
        for random_i in range(20):
            ...
            classify_fn = torch.nn.CrossEntropyLoss() if not use_reg else torch.nn.L1Loss()
            ...
            with torch.no_grad():
                with tqdm(total=total_test_size) as progress_bar:
                    ...
                    if use_reg:
                        ...
                        if r2_result < 0:
                           r2_result += 1
                        elif 0 <= r2_result < 0.05:
                            r2_result = 1 - r2_result
